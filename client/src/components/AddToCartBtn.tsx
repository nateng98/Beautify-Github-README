...
const AddToCartBtn = ({className, title, product, showPrice = true,}:{className?:string; title?:string; product?:ProductProps; showPrice?: boolean;}) => {
  ...
  const handleDeleteProduct = () => {
    if (existingProduct) {
      if (existingProduct?.quantity > 1) {
        ...
        toast.success(
          `${product?.name.substring(0, 10)} removed quantity by 1`
        );
      } else {
