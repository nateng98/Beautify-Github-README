...
const BiggerBlock = ({ banner }: BiggerBlockProps) => (
	<Block className='col-span-12 row-span-2 md:col-span-9'>
		<div className='relative'>
			<img src={banner?.image} alt="" className='w-full h-full object-cover rounded-md filter brightness-75' />
			<div className='absolute inset-0 flex flex-col justify-center px-10'>
				<h2 className='text-2xl md:text-4xl lg:text-6xl text-whiteText font-bold'>{banner?.name}</h2>
...
const SmallerBlocks = () => (
	<>
		<Block className='col-span-6 bg-red-700 md:col-span-3'>
		...
		<Block className='col-span-6 bg-darkText md:col-span-3'>
			<div className='relative grid h-full place-content-center'>
				<div
						className='absolute inset-0 w-full h-full'
						style={{
					...
					/>
					<h3 className='text-center text-2xl text-whiteText z-10'>
						<span className='font-bold'>
							AMAZING
						</span>
						<br/>smartphone offers
					</h3>
					<p className='text-center font-semibold text-2xl text-red-700 mt-4 z-10'><span className='bg-whiteText p-2'>FROM $30</span></p>
				</div>
			<div></div>
		</Block>
	</>
